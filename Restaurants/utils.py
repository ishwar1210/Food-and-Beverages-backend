...
logger = logging.getLogger("FB.utils")
...
def register_tenant_database(tenant_id, db_config):
    """
    Register a new tenant database connection
    ...
    """
    try:
        # Add to Django's database connections
        connections.databases[tenant_id] = {
        ...
        # Test connection
        with connections[tenant_id].cursor() as cursor:
            cursor.execute("SELECT 1")
        
        return True
    except Exception as e:
        print(f"Error registering tenant database: {e}")
        return False